...
import { Attribute, GroupMedicationPackagesHistory } from "common/data_interfaces";
...
interface Props { groupId: string; onSave: () => void; }

const AsignGroupMedicationCombinedForm: React.FC<Props> = ({ groupId, onSave }) => {
    ...
    const fetchPreview = async (pkg: any) => {
        if (!pkg || !configContext) return;
        try {
            const response = await configContext.axiosHelper.get(`${configContext.apiUrl}/medication_package/preview_application/${pkg._id}/${groupId}`);
            setMedicationPackagePreview(response.data.data);
        } catch (error) {
        ...
        onSubmit: async (values) => {
            ...
            try {
                const medicationsWithCosts = values.medications.map((m: any) => {
                    const pp = medicationPackagePreview?.products?.find((p: any) => p.productId === m.medication);
    ...
    const checkPackageSelected = async () => {
        ...
        await fetchPreview(selectedMedicationPackage);
    ...
    useEffect(() => {
        if (selectedMedicationPackage && groupDetails) {
    ...
    const groupAttributes: Attribute[] = [
        ...
        { label: t('common.field.name'), key: 'name', type: 'text' },
        {
            ...
            render: (_, obj) => {
                let color = "secondary";
                switch (obj.stage) { case "piglet": color = "info"; break; case "weaning": color = "warning"; break; case "fattening": color = "primary"; break; case "breeder": color = "success"; break; }
                return <Badge color={color}>{t(`pigs.stage.${obj.stage}`, { defaultValue: obj.stage })}</Badge>;
            },
        },
                ...
                <TabPane tabId={2}>
                    ...
                    <div className="d-flex justify-content-between mt-4">
                        ...
                        <Button className="btn btn-primary" onClick={async () => {
                            if (mode === 'package') { await checkPackageSelected(); }
                ...
                <TabPane tabId={3}>
                    <div className="d-flex gap-3 align-items-stretch">
                        <Card className="shadow-sm h-100">
                            <CardHeader className="bg-light fw-bold fs-5">{t('medication.assign.summary.groupCard')}</CardHeader>
                            <CardBody><ObjectDetails attributes={groupAttributes} object={groupDetails ?? {}} /></CardBody>
                        </Card>
                            ...
                            {mode === 'package' ? (
                                <>
                                    ...
                                    {medicationPackagePreview && (
                                        <Card className="shadow-sm mb-3 border-primary border-opacity-25">
                                            <CardBody className="d-flex justify-content-between align-items-center">
                                                <div>
                                                    <small className="text-muted">{t('medication.assign.preview.estimatedTotal', { defaultValue: 'Costo total estimado' })}</small>
                                                    <div className="fs-4 fw-bold text-primary">${(medicationPackagePreview.estimatedTotal ?? 0).toFixed(2)}</div>
                                                </div>
...
export default AsignGroupMedicationCombinedForm;
