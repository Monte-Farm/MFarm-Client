...
const AsignFeedingForm: React.FC<AsignFeedingFormProps> = ({ pigId, onSave }) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const feedingResponse = await configContext.axiosHelper.create(`${configContext.apiUrl}/feeding_package/asign_feeding/${userLogged.farm_assigned}/${pigId}`, feedingsData)
            await configContext.axiosHelper.create(`${configContext.apiUrl}/user/add_user_history/${userLogged._id}`, {
                event: `Alimento asignado al cerdo ${pigDetails?.code}`
            });
    ...
    const feedingValidation = Yup.object({
        ...
        quantity: Yup.number().moreThan(0, "Cantidad inválida").required("Cantidad requerida"),
    });
