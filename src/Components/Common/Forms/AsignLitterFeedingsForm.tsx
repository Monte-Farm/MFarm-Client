...
import { Attribute, GroupData, Litter, PigData } from "common/data_interfaces";
...
interface AsignLitterFeedingFormProps {
    litterId: string
    onSave: () => void;
}

const AsignLitterFeedingForm: React.FC<AsignLitterFeedingFormProps> = ({ litterId, onSave }) => {
                    ...
                    <div className="input-group">
                        <Input
                            ...
                            onChange={(e) => {
                                ...
                                const totalQuantityValue = Number(newValue * ((litterDetails?.currentMale ?? 0) + (litterDetails?.currentFemale ?? 0)));
                                const avgPerPigValue = Number(totalQuantityValue / ((litterDetails?.currentMale ?? 0) + (litterDetails?.currentFemale ?? 0)));
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const feedingResponse = await configContext.axiosHelper.put(`${configContext.apiUrl}/litter/add_feedings/${litterId}`, feedingsData)
            await configContext.axiosHelper.create(`${configContext.apiUrl}/user/add_user_history/${userLogged._id}`, {
                event: `Alimentos asignados a la camada ${litterDetails?.code}`
            });
    ...
    return (
        <>
            ...
            <SuccessModal isOpen={modals.success} onClose={() => onSave()} message={"Alimentacion asignada correctamente"} />
            <MissingStockModal isOpen={modals.missingStock} onClose={() => toggleModal('missingStock', false)} missingItems={missingItems} />
        </>
    )
}

export default AsignLitterFeedingForm;