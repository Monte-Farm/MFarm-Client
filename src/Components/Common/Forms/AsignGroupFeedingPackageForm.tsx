...
import { Attribute, FeedingPackagesEntry, GroupData, GroupFeedingPackagesHistory, GroupFeedings, PigData } from "common/data_interfaces";
...
interface AsignGroupFeedingPackageFormProps {
    groupId: string
    onSave: () => void
}

const AsignGroupFeedingPackageForm: React.FC<AsignGroupFeedingPackageFormProps> = ({ groupId, onSave }) => {
    ...
    useEffect(() => {
        if (selectedFeedingPackage) {
            const feedingsFull = selectedFeedingPackage.feedings.map((f: any) => ({
                feeding: f.feeding,
                totalQuantity: Number(f.quantity * (groupDetails?.pigCount ?? 0)),
                avgPerPig: Number((f.quantity * (groupDetails?.pigCount ?? 0)) / (groupDetails?.pigCount ?? 0))
            }))
...
export default AsignGroupFeedingPackageForm;