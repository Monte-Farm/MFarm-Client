    ...
    const fetchFeedingInfo = async () => {
        ...
        try {
            ...
            const [infoResponse, statsResponse] = await Promise.all([
                configContext.axiosHelper.get(`${configContext.apiUrl}/${FEEDING_INFO_URLS.groupFeedingInfo(groupId)}`),
                configContext.axiosHelper.get(`${configContext.apiUrl}/${FEEDING_INFO_URLS.groupFeedingStats(groupId)}`),
            ]);
