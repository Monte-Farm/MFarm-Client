...
const ViewWeanedGroups = () => {
    ...
    const [weanedStats, setWeanedStats] = useState<any>({})
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            const [groupResponse, statsResponse] = await Promise.all([
                ...
                configContext.axiosHelper.get(`${configContext.apiUrl}/group/group_alive_stats/${userLogged.farm_assigned}/weaning`),
            ])
            setWeanedGroups(groupResponse.data.data)
            setWeanedStats(statsResponse.data.data)
        } catch (error) {
        ...
        <div className="page-content">
            <Container fluid>
                ...
                <div className="d-flex gap-3 flex-wrap">
                    <KPI title="Grupos" value={weanedStats?.population?.totalGroups ?? 0} icon={FaLayerGroup} bgColor="#e8f4fd" iconColor="#0d6efd" />
                    <KPI title="Cerdos totales" value={weanedStats?.population?.totalPigs ?? 0} icon={FaPiggyBank} bgColor="#fff3cd" iconColor="#ffc107" />
                    <KPI title="Cerdos promedio por grupo" value={weanedStats?.population?.avgPigsPerGroup ?? 0} icon={FaBalanceScale} bgColor="#e6f7e6" iconColor="#28a745" />
                    <KPI title="Mínimo por grupo" value={weanedStats?.population?.minPigsPerGroup ?? 0} icon={FaArrowDown} bgColor="#f8d7da" iconColor="#dc3545" />
                    <KPI title="Máximo por grupo" value={weanedStats?.population?.maxPigsPerGroup ?? 0} icon={FaArrowUp} bgColor="#d1e7dd" iconColor="#198754" />
                    <KPI title="Machos" value={weanedStats?.population?.totalMales ?? 0} icon={FaMars} bgColor="#e7f1ff" iconColor="#0a58ca" />
                    <KPI title="Hembras" value={weanedStats?.population?.totalFemales ?? 0} icon={FaVenus} bgColor="#fde7f3" iconColor="#d63384" />
                    <KPI title="Peso promedio por cerdo (kg)" value={weanedStats?.avgWeight?.toFixed(1) ?? 0} icon={FaWeight} bgColor="#ede9fe" iconColor="#6f42c1" />
                </div>
